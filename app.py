...
def dashboard():
    categories = db.session.query(Product.category.distinct()).all()
    return render_template(
        "account/dashboard.html", title="Dashboard", categories=categories
    )
...
def details():
    ...
    return render_template(
        ...
        title="Account Details | Oasis Nourish",
...
def register():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for("home"))
    return render_template(
        "auth/register.html", form=form, title="Register | Oasis Nourish"
    )
...
def index():
    ...
    fresh_foods = (
        Product.query.filter(Product.category.like("%" + "Fresh Foods" + "%"))
        .limit(6)
        .all()
    )
    top_rated = Product.query.order_by(Product.average_ratings.desc()).limit(6).all()
    return render_template(
        "corepages/index.html",
        products=products,
        categories=categories,
...
def about():
    categories = db.session.query(Product.category.distinct()).all()
    return render_template(
        "about.html", categories=categories, title="About | Oasis Nourish"
    )
...
def terms():
    categories = db.session.query(Product.category.distinct()).all()
    return render_template(
        "terms.html", categories=categories, title="Terms of use | Oasis Nourish"
    )
...
def contact():
    if request.method == "POST":
        ...
        send_email(
            app.config["OASIS_NOURISH_ADMIN"],
            "New Message",
            "auth/email/message",
            name=name,
            message=message,
    ...
    categories = db.session.query(Product.category.distinct()).all()
    return render_template(
        "contact.html", categories=categories, title="Contact | Oasis Nourish"
    )
...
def privacy():
    categories = db.session.query(Product.category.distinct()).all()
    return render_template(
        "privacy.html", categories=categories, title="Privacy Policy | Oasis Nourish"
    )
...
def add():
    ...
    return render_template(
        ...
        admin=True,
        form=form,
        title="Add New Product | Oasis Nourish",
    )
